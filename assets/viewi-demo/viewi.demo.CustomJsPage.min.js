...
`,1)[0];if(f=L,this.options.pedantic&&(f=f.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),D.test(f)||M.test(f)||v.test(f)||q.test(n))break;if(f.search(/[^ ]/)>=m||!f.trim())l+=`
`+f.slice(m);else{if(_||g.search(/[^ ]/)>=4||D.test(g)||M.test(g)||q.test(g))break;l+=`
`+f}!_&&!f.trim()&&(_=!0),o+=L+`
